...
const strongPasswordRegex = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{4,}$/;
...
class UsersController {
    ...
    createUserAccount = async (req, res, next) => {
        ...
        if ( isSameUser )
            return res.status(400).send({ errorMessage: "이미 가입된 이메일 또는 닉네임 입니다"}); 
        if ( nickname.search(must_nickname) === -1 )
            return res.status(400).send({ errorMessage: "닉네임은 `최소 3자 이상, 특수 문자를 포함하면 안됩니다`"});
        if ( password.search(strongPasswordRegex) === -1 )
            return res.status(400).send({ errorMessage: "비밀번호는 최소 4글자 이상, 알파벳 대소문자(a~z, A~Z), 숫자(0~9), 특수문자`를 포함해야 합니다"});
